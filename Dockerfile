...
# Copy the module files and source (including vendor) into the builder
COPY . .

# Build with static linking and security flags using the vendored modules to avoid network fetches
# The local 'vendor' directory (generated by 'go mod vendor') is used with -mod=vendor
